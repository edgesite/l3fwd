...
var fdb = make(map[uint32]*FdbEntry)
...
func fillFdb(neighbor []*netlink.Neigh) error {
	for _, n := range neighbor {
		fdb[ipToUint32(n.IP)] = &FdbEntry{Neigh: n}
	}
	return nil
}
...
func installRoutes(objs *l3fwdObjects, routes []netlink.Route) error {
	...
	for _, route := range routes {
		...
		if err := objs.Routes.Put(&key, &info); err != nil {
			log.Printf("installing route %v: %v", route.Dst, err)
			continue
		}
		log.Printf("installed route %v: %+v", route.Dst, info)
	}
...
func main() {
	...
	for {
		...
		case update := <-updates:
			...
			key := RouteKey{
				DstIP: ipToUint32(update.Route.Dst.IP),
			}
			...
			case unix.RTM_DELROUTE:
				if err := objs.Routes.Delete(&key); err != nil && err != ebpf.ErrKeyNotExist {
